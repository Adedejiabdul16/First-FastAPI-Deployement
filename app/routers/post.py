...
def get_posts(db : Session = Depends(get_db), current_user : int = Depends(oauth2.get_current_user),
    ...
    #get_posts = db.query(models.Post).filter(models.Post.title.contains(search)).limit(limit).offset(skip).all()
...
def create_post(post: schemas.PostCreate, db : Session = Depends(get_db),current_user : int = Depends(oauth2.get_current_user)):
    ...
    #conn.commit()
    create_post = models.Post(user_id = current_user.id ,**post.model_dump())
    db.add(create_post)
    db.commit()
...
@router.put('/{id}', status_code=status.HTTP_201_CREATED, response_model = schemas.PostReturn)
